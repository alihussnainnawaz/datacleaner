"""
strip_validation_status_from_cleaned.py
─────────────────────────────────────────
One-off cleanup script: removes the "__validation_status__" column from
every existing *_cleaned.parquet file, across every data_type/IP, in place.

Usage (from the same directory/venv as the running server, so
output_writer.py and config.py are importable):

    python3 strip_validation_status_from_cleaned.py            # dry run, lists what it would change
    python3 strip_validation_status_from_cleaned.py --apply     # actually rewrites the files

Safe to re-run: files that don't have the column are skipped untouched.

Note: this targets *_cleaned.parquet specifically (kind="cleaned"). It does
NOT touch *_report.parquet or *_duplicates.parquet — if you need to strip
the same column from those too, use the separate scripts for each kind.
"""
...
def main(apply: bool) -> None:
    ...
    for data_type, ip_name, file_id, path in iter_all_outputs(kind="cleaned"):
        try:
